      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.emptyWrap}>
          <View style={styles.emptyIcon}>
            <Feather name="user" size={48} color="#E5E0DB" />
          </View>
          <Text style={styles.emptyTitle}>Not Signed In</Text>
          <Text style={styles.emptyDesc}>Sign in to view your profile and order history.</Text>
          <TouchableOpacity testID="profile-login-btn" style={styles.loginBtn} activeOpacity={0.8} onPress={() => router.replace('/')}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text style={styles.title}>Profile</Text>
          <TouchableOpacity testID="logout-btn" onPress={handleLogout} activeOpacity={0.8}>
        ...
        <View style={styles.linksSection}>
          {[
            ...
            { icon: 'gift', label: 'Rewards', onPress: () => router.push('/(tabs)/rewards') },
            { icon: 'map-pin', label: 'Find a Store', onPress: () => router.push('/(tabs)/stores') },
          ].map((link, i) => (
            <TouchableOpacity key={i} testID={`profile-link-${link.icon}`} style={styles.linkRow} activeOpacity={0.8} onPress={link.onPress}>
              <View style={styles.linkLeft}>
                <View style={styles.linkIcon}>
                  <Feather name={link.icon as any} size={18} color="#E67E22" />
                </View>
        ...
        <View style={styles.ordersSection}>
          ...
          {loading ? (
            ...
            <Text style={styles.noOrders}>No orders yet. Time for coffee!</Text>
          ) : (
...
const styles = StyleSheet.create({
  ...
  userCard: { marginHorizontal: 24, backgroundColor: '#FFFFFF', borderRadius: 24, padding: 28, alignItems: 'center', marginBottom: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  ...
  statValue: { fontSize: 20, fontWeight: '700', color: '#231F20' },
  ...
  linksSection: { marginHorizontal: 24, backgroundColor: '#FFFFFF', borderRadius: 16, overflow: 'hidden', marginBottom: 24 },
  ...
  orderCard: { backgroundColor: '#FFFFFF', borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  orderTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 },
  orderStore: { fontSize: 15, fontWeight: '600', color: '#231F20' },
  orderDate: { fontSize: 12, color: '#8A8A8A', marginTop: 2 },
  ...
  orderItems: { fontSize: 13, color: '#8A8A8A' },
  orderTotal: { fontSize: 16, fontWeight: '700', color: '#E67E22' },
});
