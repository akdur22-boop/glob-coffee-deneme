...
export default function TabLayout() {
  useEffect(() => {
    if (Platform.OS === 'web' && typeof window !== 'undefined') {
      ...
      if (hash && hash.includes('session_id=')) {
        ...
        if (sessionId) exchangeSession(sessionId);
      }
    }
  }, []);
  ...
  const exchangeSession = async (sessionId: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/auth/session`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: sessionId }) });
      if (res.ok) {
        const data = await res.json();
        await AsyncStorage.setItem('session_token', data.session_token);
        if (Platform.OS === 'web' && typeof window !== 'undefined') window.history.replaceState(null, '', window.location.pathname);
      }
    } catch (e) { console.log('Tab auth error', e); }
  };
